...
- **提交归属（默认禁止；人类再三明确要求时例外）**：默认 AI **不得**运行 `git commit/push/merge/rebase/reset/am`，只改工作区文件后停下等人。例外：人类**逐次明确**要求 → 先反问影响 → 再**明确确认 ≥2 次**才执行；执行须遵守密钥红线（提交信息/命令无明文 token，令牌只走内存一次性注入、`finally` 还原）、署名红线（默认不写 AI 署名）、提交信息经人类确认；push 后立即还原含 token 的 URL + 核对无残留；并记录到 WORKLOG。
